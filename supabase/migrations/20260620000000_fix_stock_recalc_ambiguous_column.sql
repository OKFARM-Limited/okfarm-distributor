-- Fix: column reference "product_id" is ambiguous in recalculate_stock
-- Root cause: the combined CTE + USING joins caused PostgreSQL to be unable
-- to resolve bare "product_id" references in the final SELECT.
-- Fix: use explicit table aliases (c.product_id, c.outlet_id) throughout.
