...
import { vendors, products, allocations, getOutletName } from '@/data/mockData';
      ...
      {vendorId && todayAlloc && (
        <Card>
          ...
          <CardContent>
                ...
                {todayAlloc.items.map(item => {
                  ...
                  return (
                    <TableRow key={item.productId}>
                      ...
                      <TableCell><Input type="number" min={0} max={item.quantity} className="w-16 h-8" value={ret || ''} onChange={e => setReturns(r => ({ ...r, [item.productId]: parseInt(e.target.value) || 0 }))} /></TableCell>
                      <TableCell><Input type="number" min={0} className="w-16 h-8" value={sp || ''} onChange={e => setSpoilage(s => ({ ...s, [item.productId]: parseInt(e.target.value) || 0 }))} /></TableCell>
            ...
            <div className="flex justify-end mt-4"><Button onClick={handleReconcile}>Save Reconciliation</Button></div>
          </CardContent>
        </Card>
      )}
